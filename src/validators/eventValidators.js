...
const fields = {
  ...
  location: z.string().trim().min(2, 'Indica o local do evento.').max(160, 'O local pode ter até 160 caracteres.'),
...
export const updateEventSchema = z.object({ params, body: z.object(fields).partial().strict().refine(value => Object.keys(value).length > 0, 'Indica pelo menos um campo para alterar.').refine(timeRange, timeError) });
